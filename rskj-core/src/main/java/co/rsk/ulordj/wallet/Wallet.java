...
import co.rsk.ulordj.core.listeners.*;
import co.rsk.ulordj.core.AbstractBlockChain;
import co.rsk.ulordj.core.LegacyAddress;
import co.rsk.ulordj.core.BlockChain;
import co.rsk.ulordj.core.BloomFilter;
import co.rsk.ulordj.core.Coin;
import co.rsk.ulordj.core.Context;
import co.rsk.ulordj.core.ECKey;
import co.rsk.ulordj.core.FilteredBlock;
import co.rsk.ulordj.core.InsufficientMoneyException;
import co.rsk.ulordj.core.Message;
import co.rsk.ulordj.core.NetworkParameters;
import co.rsk.ulordj.core.Peer;
import co.rsk.ulordj.core.PeerFilterProvider;
import co.rsk.ulordj.core.PeerGroup;
import co.rsk.ulordj.core.Sha256Hash;
import co.rsk.ulordj.core.StoredBlock;
import co.rsk.ulordj.core.Transaction;
import co.rsk.ulordj.core.TransactionBag;
import co.rsk.ulordj.core.TransactionBroadcast;
import co.rsk.ulordj.core.TransactionBroadcaster;
import co.rsk.ulordj.core.TransactionConfidence;
import co.rsk.ulordj.core.TransactionInput;
import co.rsk.ulordj.core.TransactionOutPoint;
import co.rsk.ulordj.core.TransactionOutput;
import co.rsk.ulordj.core.UTXO;
import co.rsk.ulordj.core.UTXOProvider;
import co.rsk.ulordj.core.UTXOProviderException;
import co.rsk.ulordj.core.Utils;
import co.rsk.ulordj.core.VerificationException;
import co.rsk.ulordj.core.TransactionConfidence.*;
import co.rsk.ulordj.crypto.*;
import co.rsk.ulordj.script.*;
import co.rsk.ulordj.signers.*;
import co.rsk.ulordj.utils.*;
import co.rsk.ulordj.wallet.Protos.Wallet.*;
import co.rsk.ulordj.wallet.WalletTransaction.*;
import co.rsk.ulordj.wallet.listeners.KeyChainEventListener;
import co.rsk.ulordj.wallet.listeners.ScriptsChangeEventListener;
import co.rsk.ulordj.wallet.listeners.WalletChangeEventListener;
import co.rsk.ulordj.wallet.listeners.WalletCoinsReceivedEventListener;
import co.rsk.ulordj.wallet.listeners.WalletCoinsSentEventListener;
import co.rsk.ulordj.wallet.listeners.WalletEventListener;
import co.rsk.ulordj.wallet.listeners.WalletReorganizeEventListener;
import org.slf4j.*;
 ...
 * {@link Wallet#autosaveToFile(java.io.File, long, java.util.concurrent.TimeUnit, co.rsk.ulordj.wallet.WalletFiles.Listener)}
...
public class Wallet extends BaseTaggableObject
     ...
     * {@link co.rsk.ulordj.wallet.DeterministicKeyChain#ACCOUNT_ZERO_PATH 0 hardened path}
     */
     ...
     * {@link co.rsk.ulordj.wallet.KeyChain.KeyPurpose#RECEIVE_FUNDS}. The returned key is stable until
    ...
    /**
     * An alias for calling {@link #currentKey(co.rsk.ulordj.wallet.KeyChain.KeyPurpose)} with
     * {@link co.rsk.ulordj.wallet.KeyChain.KeyPurpose#RECEIVE_FUNDS} as the parameter.
     */
    ...
    /**
     * Returns address for a {@link #currentKey(co.rsk.ulordj.wallet.KeyChain.KeyPurpose)}
     */
    ...
    /**
     * An alias for calling {@link #currentAddress(co.rsk.ulordj.wallet.KeyChain.KeyPurpose)} with
     * {@link co.rsk.ulordj.wallet.KeyChain.KeyPurpose#RECEIVE_FUNDS} as the parameter.
     */
     ...
     * {@link co.rsk.ulordj.wallet.DeterministicKeyChain}. When the parameter is
     * {@link co.rsk.ulordj.wallet.KeyChain.KeyPurpose#RECEIVE_FUNDS} the returned key is suitable for being put
    ...
    /**
     * An alias for calling {@link #freshKey(co.rsk.ulordj.wallet.KeyChain.KeyPurpose)} with
     * {@link co.rsk.ulordj.wallet.KeyChain.KeyPurpose#RECEIVE_FUNDS} as the parameter.
     */
    ...
    /**
     * Returns address for a {@link #freshKey(co.rsk.ulordj.wallet.KeyChain.KeyPurpose)}
     */
    ...
    /**
     * An alias for calling {@link #freshAddress(co.rsk.ulordj.wallet.KeyChain.KeyPurpose)} with
     * {@link co.rsk.ulordj.wallet.KeyChain.KeyPurpose#RECEIVE_FUNDS} as the parameter.
     */
     ...
     * If {@link Wallet#autosaveToFile(java.io.File, long, java.util.concurrent.TimeUnit, co.rsk.ulordj.wallet.WalletFiles.Listener)}
    ...
    /** See {@link co.rsk.ulordj.wallet.DeterministicKeyChain#setLookaheadSize(int)} for more info on this. */
    ...
    /** See {@link co.rsk.ulordj.wallet.DeterministicKeyChain#setLookaheadThreshold(int)} for more info on this. */
     ...
     * See {@link co.rsk.ulordj.wallet.DeterministicKeyChain#markKeyAsUsed(DeterministicKey)} for more info on this.
     */
     ...
     * @throws co.rsk.ulordj.core.ECKey.MissingPrivateKeyException if the seed is unavailable (watching wallet)
     */
    ...
    /**
     * Convenience wrapper around {@link Wallet#encrypt(co.rsk.ulordj.crypto.KeyCrypter,
     ...
     * {@link co.rsk.ulordj.crypto.KeyCrypterScrypt}.
     *
     ...
     * of {@link co.rsk.ulordj.core.ECKey#getCreationTimeSeconds()}. This can return zero if at least one key does
     ...
     * you can use {@link co.rsk.ulordj.utils.Threading#waitForUserCode()} to block until the future had a
    ...
    public enum MissingSigsMode {
        ...
        USE_DUMMY_SIG,
        /**
         * If signature is missing, {@link co.rsk.ulordj.signers.TransactionSigner.MissingSignatureException}
         * will be thrown for P2SH and {@link co.rsk.ulordj.core.ECKey.MissingPrivateKeyException} for other tx types.
         */
        THROW
    }
     ...
     * 
     * <p>See the docs for {@link BloomFilter#BloomFilter(int, double, long, co.rsk.ulordj.core.BloomFilter.BloomUpdate)} for a brief explanation of anonymity when using bloom
     * filters.</p>
     */
    @Override @GuardedBy("keyChainGroupLock")
     ...
     */
    public void setTransactionBroadcaster(@Nullable co.rsk.ulordj.core.TransactionBroadcaster broadcaster) {
     ...
     * @throws co.rsk.ulordj.wallet.DeterministicUpgradeRequiresPassword if key rotation requires the users password.
     */
