...
public class PrecompiledContracts {
    ...
    private static Map<RskAddress, PrecompiledContractProvider> buildContractProviders() {
        ...
        BigIntegerModexp modexp = new BigIntegerModexp();
        SamplePrecompiledContract sample = new SamplePrecompiledContract(SAMPLE_ADDR);

        contractProviders.put(ECRECOVER_ADDR, config -> ecRecover);
        contractProviders.put(SHA256_ADDR, config -> sha256);
        contractProviders.put(RIPEMPD160_ADDR, config -> ripempd160);
        contractProviders.put(IDENTITY_ADDR, config -> identity);
        contractProviders.put(SAMPLE_ADDR, config -> sample);
        contractProviders.put(BIG_INT_MODEXP_ADDR, config -> modexp);
